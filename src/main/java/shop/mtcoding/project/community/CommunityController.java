...
public class CommunityController {
    ...
    public String compBoardDetailForm(@PathVariable Integer id, Model model) {
        User sessionUser = (User) session.getAttribute("sessionUser");
        if (sessionUser == null) {
            throw new MyException("로그인 후 접근이 가능합니다.");
        }
        BoardDetailDTO boardDetailDTO = communityService.상세게시물(sessionUser.getId(), id);
        System.out.println("크 : " + boardDetailDTO.isBoardOwner());
        System.out.println("크 : " + boardDetailDTO.getTitle());
    ...
    public String compBoardUpdateForm(@PathVariable Integer id, Model model) {
        ...
        Community community = communityService.게시물내용(sessionUser.getId(), id);
